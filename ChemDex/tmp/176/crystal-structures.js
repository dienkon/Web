...
function buildSimpleHexagonal(group) {
  ...
  // Cột tâm
  addBond(group, topC, botC, 0x8cc7ff, 0.35);
}
...
function buildHCP(group) {
  ...
  addBond(group, topC, botC, 0x8cc7ff, 0.2);

  // 3 Nguyên tử ở giữa đặc trưng của HCP
  const mid = [];
  ...
  // Nối 3 hạt giữa với tâm trên và tâm dưới
  mid.forEach((p) => {
    addBond(group, p, topC, 0xffd166, 0.3);
    addBond(group, p, botC, 0xffd166, 0.3);
  });
}

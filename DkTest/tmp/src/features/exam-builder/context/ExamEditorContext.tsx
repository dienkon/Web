...
export const useExamEditorContext = () => {
  ...
  if (!ctx)
    throw new Error(
      "useExamEditorContext must be used within ExamEditorProvider",
    );
  return ctx;
};

export const ExamEditorProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  ...
  const loadExam = useCallback(async (id: string) => {
    ...
    try {
      ...
      console.log("[Firestore] READ: exams/" + id);
      ...
      let questions: Question[] = Array.isArray(rawQuestions)
        ? rawQuestions
        : [];

      if (!Array.isArray(rawSections)) {
        console.log("[Firestore] READ_MANY: exams/" + id + "/sections");
        const sectionsSnap = await getDocs(
          collection(db, `exams/${id}/sections`),
        );
        sections = sectionsSnap.docs.map(
          (d) => ({ id: d.id, ...d.data() }) as Section,
        );
      }
      if (!Array.isArray(rawQuestions)) {
        console.log("[Firestore] READ_MANY: exams/" + id + "/questions");
        const questionsSnap = await getDocs(
          collection(db, `exams/${id}/questions`),
        );
  ...
  const validate = (questions: Question[], sections: Section[]) => {
    ...
    if (questions.length === 0) {
      issues.push({
        ...
        type: "error",
        message:
          "Bài thi chưa có câu hỏi nào. Vui lòng thêm ít nhất 1 câu hỏi.",
      });
    }
      ...
      if (q.type === "single_choice" || q.type === "multiple_choice") {
        if (!q.options || q.options.length < 2) {
          issues.push({
            id: q.id,
            type: "error",
            message: `Câu ${qNum}: Cần ít nhất 2 đáp án Lựa chọn.`,
          });
        }
        if (!q.correctOptionIds || q.correctOptionIds.length === 0) {
          issues.push({
            id: q.id,
            type: "error",
            message: `Câu ${qNum}: Chưa đánh dấu đáp án đúng.`,
          });
        }
      }
      if (
        ...
        issues.push({
          id: q.id,
          type: "error",
          message: `Câu ${qNum}: Chưa nhập đáp án chấp nhận.`,
        });
      }
      if (
        q.type === "true_false" &&
        ...
        issues.push({
          id: q.id,
          type: "error",
          message: `Câu ${qNum}: Chưa có mệnh đề Đúng/Sai.`,
        });
      }
    });
  ...
  const saveExam = useCallback(async (isPublish = false) => {
    ...
    try {
      ...
      const cleanSections = sections.map((sec, idx) =>
        sanitizeForFirestore({ ...sec, order: idx, examId }),
      );
      ...
      if (estimatedSize > 900000) {
        console.warn(
          `[Firestore] Warning: Exam document size is ${estimatedSize} bytes, close to 1MiB limit.`,
        );
      }
          ...
          validationIssues: [
            {
              ...
              type: "error",
              message:
                "Kích thước đề thi quá lớn (vượt quá 1MB). Hãy giảm bớt hình ảnh, nội dung hoặc chia nhỏ đề.",
            },
          ],
        }));
        return false;
      ...
      console.log("[Firestore] WRITE: exams/" + examRef.id);
      await setDoc(examRef, cleanExamDoc, { merge: true });
        ...
        validationIssues: [
          {
            id: "system",
            type: "error",
            message: `Lỗi hệ thống khi lưu: ${error?.message || "Vui lòng kiểm tra lại mạng và thử lại."}`,
          },
        ],
      }));
      return false;
  ...
  const actions: ExamEditorActions = {
    ...
    reorderSections: (newSections) => {
      setState((s) => ({
        ...s,
        sections: newSections.map((sec, idx) => ({ ...sec, order: idx })),
      }));
      markDirty();
    },
    ...
    duplicateQuestion: (id) => {
      setState((s) => {
        ...
        if (newQ.statements) {
          newQ.statements = newQ.statements.map((st) => ({
            ...st,
            id: uuidv4(),
          }));
        }

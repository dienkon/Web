...
export default function SingleChoiceEditor({ question, update }: Props) {
  const options = question.options || [];
  const correctId = question.correctOptionIds?.[0] || null;
  ...
  const removeOption = (id: string) => {
    update({ 
      ...
      correctOptionIds: correctId === id ? [] : question.correctOptionIds
    });
  };
  ...
  return (
    <div className="space-y-3">
      ...
      {options.map((opt, idx) => (
        <div key={opt.id} className={`flex items-start gap-3 p-3 border rounded-lg transition-colors \${correctId === opt.id ? "bg-green-50 border-green-200" : "bg-white border-slate-200"}`}>
           ...
           <div className="mt-1.5 flex items-center justify-center">
              <input 
                ...
                onChange={() => update({ correctOptionIds: [opt.id] })}

...
export const getExamList = async ({
  ...
  try {
    ...
    console.log(
      `[Firestore] READ_MANY: ${EXAMS_COLLECTION} (pageSize: ${pageSize})`,
    );
    let snapshot;
    try {
      ...
      console.warn(
        "Filtered query failed, trying simple query fallback:",
        orderErr,
      );
    ...
    const items = snapshot.docs.map(
      (doc) => ({ id: doc.id, ...(doc.data() as any) }) as Exam,
    );
...
export const deleteExam = async (examId: string): Promise<void> => {
  try {
    ...
    const sectionsRef = collection(
      db,
      `${EXAMS_COLLECTION}/${examId}/sections`,
    );
    console.log(
      `[Firestore] READ_MANY: ${EXAMS_COLLECTION}/${examId}/sections (for deletion)`,
    );
    const sectionsSnap = await getDocs(sectionsRef);
    if (!sectionsSnap.empty) {
      console.log(
        `[Firestore] DELETE_BATCH: ${EXAMS_COLLECTION}/${examId}/sections (${sectionsSnap.size} docs)`,
      );
    ...
    const questionsRef = collection(
      db,
      `${EXAMS_COLLECTION}/${examId}/questions`,
    );
    console.log(
      `[Firestore] READ_MANY: ${EXAMS_COLLECTION}/${examId}/questions (for deletion)`,
    );
    const questionsSnap = await getDocs(questionsRef);
    if (!questionsSnap.empty) {
      console.log(
        `[Firestore] DELETE_BATCH: ${EXAMS_COLLECTION}/${examId}/questions (${questionsSnap.size} docs)`,
      );
    ...
    try {
      ...
      console.log(
        `[Firestore] READ_MANY: submissions (for deletion of exam ${examId})`,
      );
      const subSnap = await getDocs(subQuery);
      if (!subSnap.empty) {
        console.log(
          `[Firestore] DELETE_BATCH: submissions (${subSnap.size} docs)`,
        );
    ...
    try {
      ...
      console.log(
        `[Firestore] READ_MANY: active_sessions (for deletion of exam ${examId})`,
      );
      const sessSnap = await getDocs(sessQuery);
      if (!sessSnap.empty) {
        console.log(
          `[Firestore] DELETE_BATCH: active_sessions (${sessSnap.size} docs)`,
        );

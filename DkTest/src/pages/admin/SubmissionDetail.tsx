...
export default function SubmissionDetail() {
  ...
  const loadData = async () => {
    ...
    try {
      // 1. Fetch submission
      console.log("[Firestore] READ: submissions/" + submissionId);
      let subDoc = await getDoc(doc(db, "submissions", submissionId));
      if (!subDoc.exists() && examId) {
        console.log(
          "[Firestore] READ: exams/" + examId + "/submissions/" + submissionId,
        );
        subDoc = await getDoc(
          doc(db, `exams/${examId}/submissions`, submissionId),
        );
      }
      ...
      if (subDoc.exists()) {
        ...
        // 2. Fetch exam info
        if (targetExamId) {
          console.log("[Firestore] READ: exams/" + targetExamId);
          ...
          if (eDoc.exists()) {
            ...
            // 3. Fetch questions
            let qList: Question[] = Array.isArray((eData as any).questions)
              ? (eData as any).questions
              : [];
            if (!Array.isArray((eData as any).questions)) {
              console.log("[Firestore] READ_MANY: fallback questions query");
              const qSnap = await getDocs(
                query(
                  collection(db, `exams/${targetExamId}/questions`),
                  orderBy("order", "asc"),
                ),
              );
              qList = qSnap.docs.map(
  ...
  const handleRegradeSingle = async () => {
    ...
    try {
      const res = await regradeSingleSubmission(
        submission.id,
  ...
  const handleRegradeBatch = () => {
    const targetExamId = examId || submission?.examId;
    if (!targetExamId) return;
    setShowBatchConfirm(true);
  };

  const executeRegradeBatch = async () => {
    ...
    try {
      ...
      showToast(
        `Đã chấm lại toàn bộ ${res.totalSubmissions} bài nộp. Có ${res.changedCount} bài thay đổi điểm.`,
      );
    } catch (err: any) {
  ...
  const percentage = Math.round(
    (submission.score / (submission.maxScore || 10)) * 100,
  );
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 no-print">
        <Link
          to={
            examId ? `/admin/exams/${examId}/submissions` : "/admin/submissions"
          }
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <RotateCw
              className={`w-3.5 h-3.5 ${isRegradingSingle ? "animate-spin" : ""}`}
            />
          ...
          >
            <RefreshCw
              className={`w-3.5 h-3.5 ${isRegradingBatch ? "animate-spin" : ""}`}
            />
            {isRegradingBatch
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
          ...
          <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100">
            ...
            <span className="text-base font-bold text-slate-800 mt-0.5 block">
              {Math.floor(submission.timeSpent / 60)}p{" "}
      ...
      {submission.shuffledQuestionsSnapshot &&
        submission.shuffledQuestionsSnapshot.length > 0 && (
      ...
      {(() => {
        ...
        if (submission.subExam || submission.subExamConfigSnapshot?.enabled) {
          const snapshotIds = new Set(
            submission.shuffledQuestionsSnapshot?.map((q) => q.id) || [],
          );
        ...
        const filteredQuestions = rawActiveQuestions.filter((q) => {
          ...
          if (q.type === "single_choice") {
            isCorrect = !!(
              studentAns && q.correctOptionIds?.includes(studentAns)
            );
            ...
            isCorrect =
              stmts.length > 0 &&
              stmts.every(
                (s) => studentAns && studentAns[s.id] === s.correctAnswer,
              );
            ...
            isCorrect = !!(
              studentAns &&
              accepted.includes(String(studentAns).trim().toLowerCase())
            );
          }

          if (filterStatus === "correct" && !isCorrect) return false;
          if (filterStatus === "incorrect" && (isCorrect || !isAnswered))
            return false;
          ...
          if (searchQuery.trim()) {
            ...
            const matchOpt = q.options?.some((o) =>
              o.text?.toLowerCase().includes(queryLower),
            );
          ...
          <div className="space-y-4">
            ...
            {filteredQuestions.length === 0 ? (
              <div className="p-8 bg-white rounded-2xl border border-slate-200 text-center text-slate-500 text-sm">
                Không tìm thấy câu hỏi nào phù hợp với bộ lọc / từ khóa tìm
                kiếm.
              </div>
            ) : (
                    ...
                    {(q.type === "single_choice" ||
                      ...
                      <div className="space-y-2">
                        {q.options?.map((opt, optIdx) => {
                          ...
                          const isTeacherCorrect = q.correctOptionIds?.includes(
                            opt.id,
                          );
                          const isStudentSelected =
                            q.type === "single_choice"
                              ? studentAns === opt.id
                              : ((studentAns as string[]) || []).includes(
                                  opt.id,
                                );
                          ...
                          if (isTeacherCorrect && isStudentSelected) {
                            ...
                            containerStyle =
                              "bg-emerald-50/60 border-emerald-300 text-emerald-900 border-dashed";
                    ...
                    {q.type === "true_false" && (
                      <div className="space-y-2">
                        {q.statements?.map((stmt, sIdx) => {
                          ...
                          const isStmtCorrect =
                            studentStmtAns === stmt.correctAnswer;
      ...
      {regradeModalData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs animate-in fade-in">
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl border border-slate-200 space-y-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-900">
                  {regradeModalData.type === "single"
                    ? "Kết quả chấm lại bài thi"
                    : "Kết quả chấm lại toàn bộ"}
                </h3>
            ...
            {regradeModalData.type === "single" &&
              regradeModalData.singleResult && (
                <div className="space-y-4">
                  <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-3">
                    <div className="flex items-center justify-between text-xs text-slate-500 pb-2 border-b border-slate-200">
                      <span>Thí sinh</span>
                      <span className="font-bold text-slate-800">
                        {regradeModalData.singleResult.studentName}
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 text-center">
                      <div className="p-3 bg-white rounded-xl border border-slate-200">
                        ...
                        <span className="text-[11px] text-slate-400">
                          ({regradeModalData.singleResult.oldCorrectCount}/
                          {regradeModalData.singleResult.totalCount} câu)
                        </span>
                      </div>

                      <div
                        className={`p-3 rounded-xl border ${
                          regradeModalData.singleResult.newScore >=
                          regradeModalData.singleResult.oldScore
                      ...
                      >
                        ...
                        <span className="text-[11px] text-slate-500">
                          ({regradeModalData.singleResult.newCorrectCount}/
                          {regradeModalData.singleResult.totalCount} câu)
                        </span>
                      </div>
                    </div>
            ...
            {regradeModalData.type === "batch" &&
              regradeModalData.batchSummary && (
                <div className="space-y-4">
                  ...
                  <p className="text-xs text-slate-600 bg-slate-50 p-3 rounded-xl border border-slate-200">
                    Đã cập nhật lại toàn bộ điểm số, số câu đúng và thống kê
                    chung của đề thi thành công.
                  </p>
                </div>
              )}
      ...
      <ConfirmModal
        ...
        onConfirm={executeRegradeBatch}
        title="Xác nhận chấm lại bài thi"
        message="Bạn có chắc muốn chấm lại toàn bộ các bài nộp của đề thi này theo đáp án mới nhất?"

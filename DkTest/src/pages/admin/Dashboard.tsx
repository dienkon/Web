...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      {/* Live Proctoring Banner when students are currently taking exams */}
      {activeLiveSessions.length > 0 && (
        <div className="bg-gradient-to-r from-emerald-900 via-teal-900 to-slate-900 text-white rounded-3xl p-5 sm:p-6 shadow-xl border border-emerald-500/30 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex -space-x-2 overflow-hidden">
              {activeLiveSessions.slice(0, 4).map((s, idx) => (
                <div
                  key={s.sessionId || idx}
              ...
              {activeLiveSessions.length > 4 && (
                <div className="inline-block h-9 w-9 rounded-full ring-2 ring-emerald-400 bg-slate-800 text-emerald-300 font-bold text-xs flex items-center justify-center">
                  +{activeLiveSessions.length - 4}
                </div>
              )}
            </div>
            ...
            <Link
              ...
              className="px-5 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-slate-950 rounded-2xl text-xs font-black transition-all shadow-md hover:shadow-lg flex items-center gap-2 shrink-0 cursor-pointer"
            >
              <Eye className="w-4 h-4" />
              <span>Xem trực tiếp ({activeLiveSessions.length})</span>
            </Link>
          </div>
        </div>
      )}

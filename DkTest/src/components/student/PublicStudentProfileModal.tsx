...
export default function PublicStudentProfileModal({ isOpen, onClose, student }: Props) {
  ...
  useEffect(() => {
    ...
    const fetchPublicStats = async () => {
      setLoading(true);
      try {
        // Query submissions for this student by username or studentNameSnapshot
        ...
        let snap = await getDocs(qByUsername);
        
        // Fallback: search by studentNameSnapshot if zero results
        if (snap.empty && student.displayName) {
          const qByName = query(
            collection(db, "submissions"),
        ...
        const subs: Submission[] = snap.docs.map((d) => ({ id: d.id, ...d.data() } as Submission));

        if (subs.length > 0) {
          let maxScore = 0;
          let sumScore = 0;
          let tens = 0;
          const recents: { examTitle: string; score: number; date: any }[] = [];
          ...
          subs.forEach((s) => {
            ...
            recents.push({
              examTitle: s.examTitleSnapshot || "Bài thi khảo thí",
              score,
              date: s.submittedAt,
            });
          });

          // Sort recents by date
          recents.sort((a, b) => (b.date?.seconds || 0) - (a.date?.seconds || 0));

          setStats({
            totalExams: subs.length,
            highestScore: maxScore,
            avgScore: parseFloat((sumScore / subs.length).toFixed(2)),
            perfectTens: tens,
          });
          setRecentExams(recents.slice(0, 5));
        } else {
          // If no submissions found, show default initial state
          setStats({
            totalExams: 0,
            highestScore: 0,
            avgScore: 0,
  ...
  const studentClass = student.studentClass || "Chưa cập nhật lớp";
            ...
            <div>
              ...
              <p className="text-blue-100 text-xs font-medium flex items-center justify-center gap-2 mt-0.5">
                ...
                {student.username && (
                  <>
                    <span>•</span>
                    <span className="font-mono bg-white/15 px-2 py-0.5 rounded text-[11px]">@{student.username}</span>
                  </>
                )}
              </p>
            </div>

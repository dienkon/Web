...
export default function ExamStructureEditor({ examId }: { examId: string }) {
  ...
  const handleConfirmDeleteQuestion = async () => {
    ...
    try {
      ...
      setQuestions((prev) => ({
        ...prev,
        [questionToDelete.sectionId]: (
          prev[questionToDelete.sectionId] || []
        ).filter((oldQ) => oldQ.id !== questionToDelete.questionId),
      }));
  ...
  const handleExportSelected = () => {
    ...
    for (const sec of sections) {
      if (questions[sec.id]) {
        qsToExport.push(
          ...questions[sec.id].filter((q) => selectedQuestions.includes(q.id)),
        );
      }
    }
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      ...
                      if (newTitle && newTitle.trim()) {
                        updateSection(examId, section.id, {
                          title: newTitle.trim(),
                        }).then(() => {
                          setSections((prev) =>
                            prev.map((s) =>
                        ...
                        editingQuestion?.id === q.id ? (
                          <QuestionEditor
                            ...
                            onSaved={(savedQ) => {
                              ...
                              setQuestions((prev) => ({
                                ...prev,
                                [section.id]: prev[section.id].map((oldQ) =>
                                  oldQ.id === savedQ.id ? savedQ : oldQ,
                                ),
                              }));
                            }}
                          ...
                          <div
                            key={q.id}
                            className="flex items-start p-3 border border-slate-100 rounded-lg bg-slate-50 hover:border-blue-200 hover:bg-blue-50/50 transition-colors group"
                          >
                            <input
                              type="checkbox"
                            ...
                            <div className="flex-1 text-sm pt-0.5">
                              ...
                              <span
                                ...
                                dangerouslySetInnerHTML={{
                                  __html: q.text || "<em>Chưa có nội dung</em>",
                                }}
                              ></span>
                            </div>

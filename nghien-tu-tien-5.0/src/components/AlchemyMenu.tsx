...
export default function AlchemyMenu({
  ...
  // Initialized lazily (not from `activeRecipe`/`activeTab` directly) since
  // those consts are declared further below — referencing them here would
  // throw a temporal-dead-zone error. The sync effects below populate them
  // on first render before any code path can read a stale value.
  const activeRecipeRef = useRef<typeof ALCHEMY_RECIPES[number] | undefined>(
    undefined,
  );
  const activeTabRef = useRef<"alchemy" | "crafting">(activeTab);
  const skillStateRef = useRef({
    alchemyLevel,
    alchemyExp,
    craftingLevel,
    craftingExp,
  });
  // Flag set by the interval tick; consumed by an effect so that the
  // actual setState calls happen during a commit, never inside another
  // component's render or inside a setState updater function.
  ...
  }, [alchemyLevel, alchemyExp, craftingLevel, craftingExp]);
  ...
  const handleRefine = () => {
    ...
    // Trừ nguyên liệu ngay lập tức để UI cập nhật tức thì, không chờ
    // thanh tiến trình chạy xong. Snapshot công thức/inventory hiện tại
    // vào ref để finishRefine() dùng đúng dữ liệu đã trừ này khi tính
    // kết quả (thành công/thất bại, vật phẩm, EXP).
    let inventoryAfterDeduction = inventoryRef.current;
    activeRecipe.ingredients.forEach((ing) => {
      inventoryAfterDeduction = removeInventoryItem(
        inventoryAfterDeduction,
        ing.itemId,
        ing.count,
      );
    });
    inventoryAfterDeduction = normalizeInventoryItems(inventoryAfterDeduction);

    inventoryRef.current = inventoryAfterDeduction;
    setInventory(inventoryAfterDeduction);
    activeRecipeRef.current = activeRecipe;
    activeTabRef.current = activeTab;
    ...
    refineIntervalRef.current = setInterval(() => {
      setRefineProgress((prev) => {
        ...
        if (next >= 100) {
          ...
          // Defer the actual finishRefine() side effects (multiple
          // setState calls) to an effect instead of calling them here,
          // since this updater fn must stay pure.
  ...
  // Runs the actual finishRefine() logic outside of render/updater context,
  // once refineProgress has visibly hit 100% and the interval flagged completion.
  useEffect(() => {
  ...
  const finishRefine = () => {
    ...
    try {
      // Nguyên liệu đã được trừ ngay khi bấm nút ở handleRefine(), nên ở
      // đây chỉ tính kết quả thành công/thất bại dựa trên inventory hiện
      // tại (đã trừ) — không trừ lại lần nữa.
      ...
      const { alchemyLevel, alchemyExp, craftingLevel, craftingExp } =
        skillStateRef.current;
      const currentLevel = tab === "alchemy" ? alchemyLevel : craftingLevel;
      const levelBonus = (currentLevel - recipe.reqSkillLevel) * 0.02;
      const finalRate = Math.min(0.99, recipe.successRate + levelBonus);
      ...
      if (roll < finalRate) {
        ...
        if (qRoll < 0.05) {
          quality = "Cực phẩm";
          countAwarded = tab === "alchemy" ? 3 : 1;
        } else if (qRoll < 0.3) {
          quality = "Trung phẩm";
          countAwarded = tab === "alchemy" ? 2 : 1;
        }

        if (recipe.type === "alchemy") {
          const template = getItemTemplate(recipe.resultId);
          if (template) {
            nextInventory = normalizeInventoryItems(
              addInventoryItem(
                nextInventory,
                {
                  ...template,
          ...
          let slot: any = "weapon";
          if (recipe.resultId === "phi_kiem") slot = "weapon";
          else if (recipe.resultId === "ho_lo") slot = "artifact";
          else if (recipe.resultId === "bao_thap") slot = "artifact";
          else if (recipe.resultId === "ngoc_boi") slot = "necklace";
          else if (recipe.resultId === "dong_hoang_chung")
            slot = "artifact";
          ...
          const newGear = createEquipment(
            ...
            `${getItemTemplate(recipe.resultId)?.name || "Vật phẩm"}`,
            slot,
            gearRarity,
          ...
          nextInventory = normalizeInventoryItems(
            addInventoryItem(nextInventory, newGear, 1),
          );
        }

        const xpGained = recipe.reqSkillLevel * 15;
        if (tab === "alchemy") {
        ...
        setRefineLog((prev) => [
          `✅ Luyện thành công: [${getItemTemplate(recipe.resultId)?.name || "Vật phẩm"}] - Phẩm chất: ${quality}! (+${xpGained} EXP kỹ năng)`,
          ...prev,
        ]);
      } else {
        playSound("failure");

...
const buildZaloSenderIdKey = (uid?: string | null) =>
  keys.zaloSenderIdPrefix + normalizeUid(uid || activeMailOwnerUid || "global");

const normalizeZaloSenderId = (value?: string | null) =>
  String(value || "").trim();
...
export const setCachedZaloSenderId = (uid?: string | null, senderId?: string | null) => {
  const key = buildZaloSenderIdKey(uid);
  const normalized = normalizeZaloSenderId(senderId);
  if (!key) return;
  if (!normalized) {
...
export const getCachedZaloSenderId = (uid?: string | null) => {
  const key = buildZaloSenderIdKey(uid);
  return normalizeZaloSenderId(safeStorage.get(key, ""));
};
...
const shouldForwardMailToZalo = (mail: ReturnType<typeof normalizeMail>) => {
  const hay = `${mail.title}
${mail.content}
${mail.eventType || ""}`.toLowerCase();
  return (
    hay.includes("pk") ||
    hay.includes("cướp") ||
    hay.includes("cuop") ||
    hay.includes("song tu") ||
    hay.includes("songtu") ||
    hay.includes("trộm rau") ||
    hay.includes("trom rau") ||
    hay.includes("rob_spirit") ||
    hay.includes("dual_cultivate") ||
    hay.includes("steal_vegetable")
  );
};

const broadcastGlobalMailPopup = (mail: ReturnType<typeof normalizeMail>) => {
  if (typeof window === "undefined") return;
  const message = `${mail.title}\n${mail.content}`;
  window.dispatchEvent(
    new CustomEvent("game-popup", {
      detail: {
        message,
        type: mail.type === "combat" ? "error" : "info",
      },
    }),
  );
};
...
export const addMailToInbox = (
  ...
  if (shouldForwardMailToZalo(safeMail)) {
    broadcastGlobalMailPopup(safeMail);

    const zaloSenderId = getCachedZaloSenderId(targetUid);
    if (zaloSenderId) {
      void sendZaloPersonalMessage({
        toUid: zaloSenderId,
        toName: safeMail.recipientName || safeMail.toName || "",
        senderId: safeMail.recipientUid || targetUid,
        recipientUid: targetUid,
        title: safeMail.title,
        content: safeMail.content,
        type: safeMail.type,
        eventType: safeMail.eventType,
      }).catch((error) => {
        console.error("sendZaloPersonalMessage failed:", error);
      });
    }
  }
...
export const sendZaloPersonalMessage = async (payload: {
  ...
  const res = await fetch(endpoint, {
    ...
    body: JSON.stringify({
      ...
      senderId: payload.senderId || payload.toUid,

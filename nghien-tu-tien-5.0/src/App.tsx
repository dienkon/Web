...
import { 
  ...
  setActiveMailOwner,
  addMailToInbox,
  getCachedZaloSenderId,
  setCachedZaloSenderId,
} from './lib/firebase';
...
export default function App() {
  ...
  const [zaloSenderIdInput, setZaloSenderIdInput] = useState<string>('');
  ...
  useEffect(() => {
    ...
    window.addEventListener("game-popup", handler);

    return () => window.removeEventListener("game-popup", handler);
  }, []);
...
function buildGameSave(playerOverride?: PlayerCharacter): GameSave {
  const currentPlayer = (playerOverride ?? player) as PlayerCharacter;
  return {
    player: {
      ...currentPlayer,
      zaloSenderId: String((currentPlayer as any).zaloSenderId || zaloSenderIdInput || ''),
    } as PlayerCharacter,
    inventory,
    skills,
    companions,
...
function applyGameSave(parsed: Partial<GameSave>) {
  if (parsed.player) {
    ...
    setZaloSenderIdInput(String((parsed.player as any).zaloSenderId || ''));
  }
...
useEffect(() => {
  ...
  setZaloSenderIdInput(
    String((player as any)?.zaloSenderId || getCachedZaloSenderId(user.uid) || ''),
  );
  setCachedZaloSenderId(
    user.uid,
    String((player as any)?.zaloSenderId || getCachedZaloSenderId(user.uid) || ''),
  ...
  const handleSaveZaloSenderId = async () => {
    const normalized = String(zaloSenderIdInput || '').trim();
    setPlayer((prev) => ({
      ...prev,
      zaloSenderId: normalized,
    }) as any);

    if (user?.uid) {
      setCachedZaloSenderId(user.uid, normalized);
    }

    await saveGame();
    alert(
      normalized
        ? `Đã lưu Zalo Sender ID: ${normalized}`
        : 'Đã xoá Zalo Sender ID khỏi hồ sơ.',
    );
  };
        ...
        <div className="flex-1 overflow-y-auto bg-stone-900/30 scrollbar-none pb-20" id="main_tab_viewport">
          ...
          {activeTab === 'settings' && (
            <div className="p-4 text-left space-y-4 text-stone-200" id="tab_view_settings">
              ...
              {/* Zalo bot settings */}
              <div className="bg-stone-900 border border-stone-800 p-3 rounded-lg text-xs space-y-2" id="settings_zalo_box">
                <p className="text-stone-400 font-bold uppercase flex items-center gap-1.5">
                  <CloudLightning size={12} className="text-cyan-400" /> Zalo Bot Sender ID
                </p>
                <input
                  value={zaloSenderIdInput}
                  onChange={(e) => setZaloSenderIdInput(e.target.value)}
                  placeholder="Nhập sender ID / UID Zalo nhận thông báo"
                  className="w-full rounded-lg bg-stone-950 border border-stone-700 px-3 py-2 text-[11px] outline-none text-stone-200 placeholder:text-stone-600 font-mono"
                />
                <button
                  onClick={handleSaveZaloSenderId}
                  className="w-full py-2 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-stone-950 font-black text-[11px] active:scale-95 transition-all"
                >
                  Lưu Sender ID Zalo
                </button>
                <p className="text-[10px] text-stone-500 leading-relaxed">
                  Mỗi thông báo PK / cướp / trộm rau / song tu sẽ được đẩy sang bot Zalo theo sender ID này.
                </p>
              </div>

...
export const getItemStackKey = (item: Partial<GameItem> | null | undefined): string => {
  if (!item) return 'empty';
...
export const normalizeInventoryItems = (items: GameItem[] = []): GameItem[] => {
  ...
  for (const raw of items || []) {
    if (!raw) continue;
    const item = { ...raw };
    item.count = Math.max(0, Number((item as any).count ?? item.quantity ?? 0));
...
export const addInventoryItem = (items: GameItem[], item: GameItem, count = item.count || 1): GameItem[] => {
  ...
  if (idx >= 0) {
    ...
    next.push({ ...item, count, quantity: count });
  }

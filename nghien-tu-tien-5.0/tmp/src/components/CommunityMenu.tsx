...
import { showPopup } from "../services/popupBus";
...
export default function CommunityMenu({
  ...
  const cooldownStorageKey = (action: "rob" | "dual" | "pk") =>
    `${COOLDOWN_STORAGE_PREFIX}:${String(getAuth().currentUser?.uid  || "guest")}:${action}`;
  ...
  const processedCombatEventsKey = () =>
    `${COOLDOWN_STORAGE_PREFIX}:${String(getAuth().currentUser?.uid  || "guest")}:combat_processed`;
const handlingCombatEventsRef = useRef(new Set<string>());
 ...
 const queueCombatEvent = async (
  ...
  const applyIncomingCombatEvent = async (event: any, sourcePlayer?: any) => {
    ...
    const selfUid = String(getAuth().currentUser?.uid || "");
    ...
    if (handlingCombatEventsRef.current.has(eventId)) return false;
    handlingCombatEventsRef.current.add(eventId);
    ...
    console.log("APPLYING INCOMING COMBAT EVENT:", eventId, targetPayload);
    console.log(targetPayload.content);

    if (targetPayload?.content) {
      console.log("ALERT CONTENT:", targetPayload.content);
      window.dispatchEvent(
        new CustomEvent("game-popup", {
          detail: {
            message: targetPayload.content,
          },
        }),
      );
    }
    ...
    if (committed) {
      try {
        ...
        // Xóa event đã xử lý khỏi Firestore
      
      } catch (error) {
        handlingCombatEventsRef.current.delete(eventId);
  ...
  const flushIncomingCombatEvents = async () => {
    ...
    for (const item of pending) {
      const ok = await applyIncomingCombatEvent(item.event, item.sourcePlayer);

      if (ok) {
        await updateDoc(doc(db, "players", selfUid), {
          combatEvents: arrayRemove(item.event),
        });
      }
    }
  };
  ...
  useEffect(() => {
    if (!getAuth().currentUser?.uid  || allPlayers.length === 0) return;
    void flushIncomingCombatEvents();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [getAuth().currentUser?.uid ?? "", allPlayers]);

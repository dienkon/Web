...
import { getNextLevelExp, playSound } from '../utils/gameData';
import { User, Users, Star, Sparkles, Heart, Sparkle } from 'lucide-react';
...
export default function CompanionMenu({
  ...
  const songTuPartnersList = [
    { id: 'bach_ngung_bang', name: '❄️ Bạch Ngưng Băng', desc: 'Linh Thể Băng Tộc, tính cách cao ngạo lạnh lùng nhưng tâm tư thuần khiết.', bonus: 'Buff cộng: 0' },
    { id: 'to_nhu_ngoc', name: '🌸 Tô Như Ngọc', desc: 'Mị Linh Tiên Tử, am hiểu Song Tu Cực Lạc Tâm Pháp bậc nhất nhân gian.', bonus: 'Buff cộng: 0' },
    { id: 'diap_thanh', name: '🌿 Diệp Thanh', desc: 'Thần y mộc linh, giúp hồi phục chân khí kinh mạch ấm áp dồi dào.', bonus: 'Buff cộng: 0' },
    { id: 'tieu_viem', name: '🔥 Tiêu Viêm', desc: 'Viêm Đế chí tôn, giúp tôi luyện gân cốt nóng bỏng cường hóa thân thể.', bonus: 'Buff cộng: 0' }
  ] as const;

  type SongTuType = 'santhien' | 'linhgiac' | 'trangdao';

  const songTuRitualConfig: Record<
    SongTuType,
    {
      name: string;
      costRate: number;
  ...
  > = {
    santhien: {
      name: '🤝 Tâm Linh Tương Thông',
      costRate: 2,
      expRate: 0.1,
      goldReward: 200,
      desc: 'Ngưng thần nhắm mắt dạo thần khí.',
      confirmText: 'Đạo hữu có đồng ý thực hiện Tâm Linh Tương Thông không?'
    },
    linhgiac: {
      name: '🌌 Linh Hồn Hòa Quyện',
      costRate: 8,
      expRate: 0.3,
      goldReward: 600,
      desc: 'Nâng niu tiên thạch dung nhập tu vi.',
      confirmText: 'Đạo hữu có đồng ý thực hiện Linh Hồn Hòa Quyện không?'
    },
    trangdao: {
      ...
      expRate: 0.8,
      goldReward: 1800,
      desc: 'Hồi thần phi thăng đại lân thiên đỉnh.',
      confirmText: 'Đạo hữu có đồng ý thực hiện Đại Đạo Quy Nhất không?'
    }
  };

  const getSongTuBaseExp = () => Math.max(1, Number(getNextLevelExp(player.realmLevel)) || 0);

  const getSongTuRuntimeValues = (type: SongTuType) => {
    const baseExp = getSongTuBaseExp();
    const ritual = songTuRitualConfig[type];

    return {
      ...ritual,
      baseExp,
      cost: Math.max(1, Math.ceil(baseExp * ritual.costRate)),
      expReward: Math.max(1, Math.ceil(baseExp * ritual.expRate))
    };
  };

  const applyPlayerStatDelta = (
    delta: {
      hp?: number;
      atk?: number;
      def?: number;
    ...
    setPlayer((prev) => {
      ...
      const adjustStat = (preferredKeys: string[], value?: number) => {
        if (typeof value !== 'number' || Number.isNaN(value) || value === 0) return;

        const existingKey = preferredKeys.find((key) => typeof next[key] === 'number');
        const keyToUse = existingKey ?? preferredKeys[0];
        const currentValue = typeof next[keyToUse] === 'number' ? next[keyToUse] : 0;
        next[keyToUse] = currentValue + value * direction;
      };

      adjustStat(['maxHp', 'hp'], delta.hp);
      adjustStat(['atk', 'attack'], delta.atk);
      adjustStat(['def', 'defense'], delta.def);
      adjustStat(['atkSpeed', 'attackSpeed'], delta.atkSpeed);
      adjustStat(['evasion'], delta.evasion);

      return next;
    });
  };

  const applyCompanionBonusToPlayer = (comp: Companion, direction: 1 | -1) => {
  ...
  const applyBeastBonusToPlayer = (bonusStats: SpiritBeast['bonusStats'], direction: 1 | -1) => {
    applyPlayerStatDelta(
      {
        hp: bonusStats.maxHp,
        atk: bonusStats.atk,
        def: bonusStats.def,
        atkSpeed: bonusStats.atkSpeed,
        evasion: bonusStats.evasion
      },
      direction
    );
  };
  ...
  const handlePerformSongTu = async (type: SongTuType) => {
    const ritual = getSongTuRuntimeValues(type);


     const ok = await confirm({
       title: "Song tu không ku",
       message: `${ritual.confirmText}\n\nChi phí: ${ritual.cost.toLocaleString()} Linh thạch\nNhận: +${ritual.expReward.toLocaleString()} Tu vi và +${ritual.goldReward.toLocaleString()} Vàng`,
     });
    ...
    if (player.spiritStones < ritual.cost) {
    ...
    setPlayer((prev) => ({
      ...prev,
      spiritStones: prev.spiritStones - ritual.cost,
      gold: prev.gold + ritual.goldReward,
      cultivation: prev.cultivation + ritual.expReward
    }));

    setTimeout(() => {
      setSongTuAnimate(false);
      const newLog = `[${new Date().toLocaleTimeString()}] Đạo hữu cùng ${partner?.name} thực hiện nghi thức Song Tu [${ritual.name}], nhận được +${ritual.expReward.toLocaleString()} Tu vi và +${ritual.goldReward.toLocaleString()} Vàng đút túi!`;
  ...
  const handleToggleCompanion = (id: string) => {
    ...
    applyCompanionBonusToPlayer(target, willActivate ? 1 : -1);
  ...
  const handleStarUpCompanion = (id: string) => {
    ...
    if (target.stars >= 5) {
      alert('Tiên Hữu này đã đạt tối đa 5 sao!');
      return;
    }
    ...
    const nextStar = Math.min(5, target.stars + 1);
    const afterBonus = {
      hp: Math.round(target.hp * 1.3),
      atk: Math.round(target.atk * 1.3),
      def: Math.round(target.def * 1.3)
    };

    if (target.active) {
      applyPlayerStatDelta(beforeBonus, -1);
      applyPlayerStatDelta(afterBonus, 1);
    }

    setInventory((prev) =>
      prev
        .map((i) => (i.id === 'manh_dong_hanh' ? { ...i, count: i.count - 3 } : i))
        .filter((i) => i.count > 0)
    );
  ...
  const handleToggleBeast = (id: string) => {
    ...
    const target = spiritBeasts.find((b) => b.id === id);
    if (!target) return;

    const currentlyActive = target.active;
    const previousActive = spiritBeasts.find((b) => b.active);

    setSpiritBeasts((prev) =>
      prev.map((b) => ({
        ...b,
        active: b.id === id ? !b.active : false
      }))
    );

    if (currentlyActive) {
      applyBeastBonusToPlayer(target.bonusStats, -1);
    } else {
      if (previousActive && previousActive.id !== id) {
        applyBeastBonusToPlayer(previousActive.bonusStats, -1);
      }
      applyBeastBonusToPlayer(target.bonusStats, 1);
    }
  ...
  const handleStarUpBeast = (id: string) => {
    ...
    if (target.stars >= 5) {
      alert('Thần thú này đã đạt tối đa 5 sao!');
      return;
    }
    ...
    const beforeBonus = { ...target.bonusStats };
    const nextStar = Math.min(5, target.stars + 1);
    const nextBonus = { ...target.bonusStats };
    ...
    if (nextBonus.atkSpeed) nextBonus.atkSpeed = Math.round(nextBonus.atkSpeed * 1.4);
    if (nextBonus.evasion) nextBonus.evasion = Math.round(nextBonus.evasion * 1.4);

    if (target.active) {
      applyBeastBonusToPlayer(beforeBonus, -1);
      applyBeastBonusToPlayer(nextBonus, 1);
    }

    setInventory((prev) =>
      prev
        .map((i) => (i.id === 'manh_linh_thu' ? { ...i, count: i.count - 4 } : i))
        .filter((i) => i.count > 0)
    );
  ...
  const songTuRituals = (['santhien', 'linhgiac', 'trangdao'] as SongTuType[]).map((type) => {
    const values = getSongTuRuntimeValues(type);
    return {
      type,
      ...values
    };
          ...
          <div className="space-y-4 text-left" id="songtu_pane">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                {songTuRituals.map((ritual) => (
                  <button
                    key={ritual.type}
                    disabled={songTuAnimate}
                    onClick={() => handlePerformSongTu(ritual.type)}
                  ...
                  >
                    ...
                    <div className="mt-3.5 space-y-1">
                      <div className="flex justify-between text-[8px] font-mono">
                        ...
                        <span className="text-cyan-400 font-bold">+{ritual.expReward.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between text-[8px] font-mono">
                        <span className="text-stone-400">Vàng:</span>
                        <span className="text-amber-400 font-bold">+{ritual.goldReward.toLocaleString()}</span>
                      </div>
                    </div>

                    <div className="mt-2 pt-2 border-t border-stone-800 flex justify-center">
                      <span className="text-[9px] font-black text-rose-400">
                        {ritual.cost.toLocaleString()} Linh thạch
                      </span>
                    </div>
                  </button>
                ))}

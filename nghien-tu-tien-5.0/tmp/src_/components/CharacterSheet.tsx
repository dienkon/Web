...
export default function CharacterSheet({
  ...
  const getItemStackKey = (
    item?: Pick<
      GameItem,
      "id" | "rarity" | "type" | "equipmentSlot" | "enhancementLevel"
    > | null,
  ) => {
  ...
  const handleSellItem = async (item: GameItem, qtyOverride?: number) => {
    ...
    let sellPrice = 20;
    if (item.rarity === "Thần Thoại" || item.rarity === "Tiên Khí")
      sellPrice = 1200;
    else if (item.rarity === "Đỏ") sellPrice = 600;
    else if (item.rarity === "Cam") sellPrice = 300;
    else if (item.rarity === "Tím") sellPrice = 150;
    else if (item.rarity === "Lam") sellPrice = 75;
    else if (item.rarity === "Lục") sellPrice = 40;

    const qty = Math.max(1, Math.min(item.count, qtyOverride ?? sellQuantity));
    const rarityScale = getRarityMultiplier(item.rarity);
    const total = Math.round(sellPrice * qty * rarityScale);
